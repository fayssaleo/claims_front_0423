<template>
  <div style="padding: 5px; padding-top: 5%">
    ...
    >
      <template v-slot:item="{ item }">
        <tr class="claims_tr">
          ...
          <td v-else class="cursor">
            <v-chip color="#FF9800" class="white--text cursor">
              NOT YET
            </v-chip>
          </td>
          ...
          <td v-if="item.getReinvoiced == 'TO PROCEED'" class="cursor">
            <v-chip color="#FF9800" class="white--text cursor"
              >TO PROCEED</v-chip
            >
          </td>
          <td v-else-if="item.getReinvoiced == 'REINVOICED'" class="cursor">
            <v-chip color="#FF9800" class="white--text cursor"
              >REINVOICED</v-chip
            >
          </td>
          <td v-else-if="item.getReinvoiced == 'PAID'" class="cursor">
            <v-chip color="#FF9800" class="white--text cursor">PAID</v-chip>
          </td>
          <td v-else class="cursor">
            <v-chip color="#FF9800" class="white--text cursor">NOT YET</v-chip>
          </td>
          ...
          <td>
            <v-row class="ma-0 pa-0">
              <v-col
                cols="12"
                class="ma-0 pa-0"
                v-if="getUserActive.fonction.name != 'accountable'"
                ><v-btn
                  color="primary"
              ...
              >
                <v-btn
                  v-if="item.getReinvoiced == 'TO PROCEED'"
                  color="#009688"
                  @click="reinvoicedDialogeF(item)"
                  class="m-2 mt-2 btnAction white--text"
                  style="height: 26px; min-width: 100%; margin-bottom: 7px"
                >
                  <v-icon medium> mdi-receipt-text-check </v-icon>
                </v-btn></v-col
              >
              <v-col
                cols="12"
              ...
              >
                <v-btn
                  v-if="item.getReinvoiced == 'REINVOICED'"
                  color="#4CAF50"
                  @click="paidDialogeF(item)"
                  class="m-2 mt-2 btnAction white--text"
                  style="height: 26px; min-width: 100%; margin-bottom: 7px"
                >
                  <v-icon medium> mdi-bank-check </v-icon>
                </v-btn></v-col
              >
      ...
      <template v-slot:top>
        <v-toolbar flat>
            ...
            <v-card>
              ...
              <v-card-actions>
                ...
                <v-btn color=" darken-1" @click="treatedDialog = false"
                  >Cancel</v-btn
                >
                <v-btn color="#009688" class="white--text" @click="treatedF()"
                  >OK</v-btn
                >
                <v-spacer></v-spacer>
              </v-card-actions>
          ...
          <v-dialog v-model="reinvoicedDialog" max-width="600px">
            <v-card>
              <v-card-title class="text-h5"
                >Are you sure you want to turn this item to 'Reinvoiced'
                ?</v-card-title
              >
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color=" darken-1" @click="reinvoicedDialog = false"
                  >Cancel</v-btn
                >
                <v-btn
                  color="#009688"
          ...
          <v-dialog v-model="paidDialog" max-width="600px">
            <v-card>
              <v-card-title class="text-h5"
                >Are you sure you want to turn this item to 'Paid'
                ?</v-card-title
              >
              <v-card-text>
                <v-row justify="space-around">
                  <v-menu
                    v-model="paidDateTimeShow"
                    :close-on-content-click="false"
                    :nudge-right="40"
                    transition="scale-transition"
                    offset-y
                  ...
                  >
                    <template v-slot:activator="{ on, attrs }">
                      <v-text-field
                        v-model="paidDateTimeText"
                        label="Select the reimbursment date"
                        prepend-icon="mdi-calendar"
                        readonly
                        v-bind="attrs"
                        v-on="on"
                    ...
                    <v-date-picker
                      v-model="paid.date_of_reimbursement"
                      @input="onInput2"
                      color="primary"
                      scrollable
                      ref="picker"
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color=" darken-1" @click="paidDialog = false"
                  >Cancel</v-btn
                >
                <v-btn
                  :disabled="
                    paid.date_of_reimbursement == '' ||
                    paid.date_of_reimbursement == null
                  "
                  color="#009688"
                  class="white--text"
                  @click="paidF()"
          ...
          >
            ...
            <v-card>
              <v-toolbar dark color="primary">
                ...
                <v-toolbar-title>{{
                  (selectedClaimToShow.ClaimOrIncident == "Incident"
                    ? "INCIDENT : "
                    : "CLAIM : ") + selectedClaimToShow.claimSerial
                }}</v-toolbar-title>
                <v-spacer></v-spacer>
                <v-toolbar-items>
                  <span
                  ...
                  <v-btn
                    ...
                    >As
                    <v-icon size="x-large" class="ml-2">
                      mdi-file-pdf-box
                    </v-icon></v-btn
                  >
                  <v-btn
                    color="#009688"
                    class="white--text"
                    @click="downloadAsExcel(getCLAIMS_TO_DOWNLOAD)"
                    >As
                    <v-icon size="x-large" class="ml-2">
                      mdi-microsoft-excel
                    </v-icon></v-btn
          ...
          >
            ...
            <v-card>
              <v-toolbar dark color="primary" class="sticky-toolbar">
                ...
                <v-toolbar-title
                  >Download ( {{ selectedClaims.length }} Selected
                  )</v-toolbar-title
                >
                <v-spacer></v-spacer>
                <span
                  style="cursor: pointer; font-size: 20px; margin-right: 20px"
                  @click="
                    selectedClaims.length == afterFiltedArray.length
                      ? (selectedClaims = [])
                      : (selectedClaims = [...afterFiltedArray])
                  "
                >
                  Select all
                </span>
              ...
              <card-text>
                <v-checkbox-group v-model="selectedClaims" class="checklist">
                  <v-checkbox
                    ...
                    :value="
                      selectedClaims.filter((e) => e.id == claim.id).length > 0
                    "
                    @click="toggleSelectedClaims(claim)"
                    :class="{
                      checked:
                        selectedClaims.filter((e) => e.id == claim.id).length >
                        0,
                    }"
                  ></v-checkbox>
                </v-checkbox-group>
              ...
              <v-card-actions class="sbicky-toolbar" dark color="primary">
                ...
                <v-btn
                  color=" red"
                  class="white--text"
                  @click="setSelectedClaimsToShowAction(selectedClaims)"
                  >As
                ...
                <v-btn
                  color="#009688"
                  class="white--text"
                  @click="downloadAsExcel(selectedClaims)"
                  >As
                ...
                >
                <v-spacer></v-spacer>
                <v-btn color="darken-1" @click="showToDownlodPDFDialog = false"
                  >Close</v-btn
                >
              </v-card-actions>
            </v-card>
    ...
    <vue-html2pdf
      ...
      :preview-modal="true"
      :filename="'claims-' + new Date().toLocaleString() + '.pdf'"
      :pdf-quality="2"
    ...
    >
      <section style="width: 1100px" slot="pdf-content">
        <ClaimHtml :claims="selectedClaimsToShow" />
      </section>
    </vue-html2pdf>
  </div>
</template>
<script>
import ClaimHtml from "./ClaimHtml.vue";
...
export default {
  components: { ClaimHtml, ClaimDetails, VueHtml2pdf },
  data: () => ({
    selectedClaimsToShow: [],
    selectedClaimToShow: {},
    selectedClaims: [],
  ...
  methods: {
    ...
    closedDialogeF(item) {
      ...
      this.closed.close_comment = item.close_comment;
    ...
    reinvoicedDialogeF(item) {
      this.reinvoiced.id = item.id;

      this.reinvoicedDialog = true;
    },
    paidDialogeF(item) {
      this.paid.id = item.id;
      this.paid.date_of_reimbursement = "";
      this.paidDateTimeText = "";
    ...
    reinvoicedF() {
      this.reinvoicedClaimAction(this.reinvoiced)
        .then(() => {
          this.claims = [...this.getclaims];
          for (let index = 0; index < this.claims.length; index++) {
            this.claims[index].claimSerial = this.getClaimSerialNumber(
    ...
    paidF() {
      this.paidClaimAction(this.paid)
        .then(() => {
          this.claims = [...this.getclaims];
          for (let index = 0; index < this.claims.length; index++) {
            this.claims[index].claimSerial = this.getClaimSerialNumber(
    ...
    onInput2(dateStr) {
      ...
      const year = dateStr.split("-")[0];
      this.paidDateTimeText = `${month}/${day}/${year}`;
      this.paidDateTimeShow = false;
    },
    reinitialize() {
    ...
    showDetailsAction(item) {
      this.setindexClaimsByIdsAction([item.id])
        .then(() => {
          ...
          this.selectedClaimsToShow = this.getCLAIMS_TO_DOWNLOAD;
          this.showDetailsDialog = true;
        })
        .catch((e) => {});
    },
    setSelectedClaimsToShowAction(Ids) {
      this.setModuleShowToTrueAction();
      this.setindexClaimsByIdsAction(Ids.map((e) => e.id))
        .then(() => {
          this.selectedClaimsToShow = this.getCLAIMS_TO_DOWNLOAD;
    ...
    toggleSelectedClaims(claim) {
      ...
      } else this.selectedClaims.push(claim);
